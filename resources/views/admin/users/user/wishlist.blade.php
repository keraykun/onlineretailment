...
<section class="landing flex-shrink-1 flex  flex-col items-center justify-around w-full" style="padding-bottom: 25px;">
    ...
    <article class="report-title w-full p-4">
       <u style="margin-right: 10px;">{{ $orders->count() }}</u>  WishList by  <u style="margin-left: 10px;"> {{ $data->name }}</u>
    </article>
    <table id="customers">
        <tr>
          <th></th>
        ...
        <tr>
            <td class="td-image">
                <div class="td-image-size">
                    <img src="{{ asset('storage/order/'.$order->product->image->name) }}" alt="">
                </div>
            </td>
            <td>
            ...
            <td width="10%">{{ number_format($order->product->price,2) }}</td>
            <td>
                <p>{{ $order->status }}</p>
            </td>
            <td width="">
