...
<section class="report-box">
    ...
    <article class="report-list">
        <div style="display: flex;">
            <a href="{{ route('admin.report.product.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
            <a href="#report-box-reply"><i class="fa fa-envelope" aria-hidden="true"></i></a>
             <form method="POST" action="{{ route('admin.report.product.destroy',$report->id) }}">
                @csrf
                {{ method_field('delete') }}
                <button type="submit"><i class="fa fa-trash"></i></button>
             </form>
             <form method="POST" action="{{ route('admin.report.product.update',$report->id) }}">
                @csrf
                {{ method_field('PATCH') }}
...
<section class="report-box">
    <article class="report-list" style="height: 0%;">
        <div class="report-header-reply">
          ...
          <div style="display: flex;">
            @can('delete',$reply)
            <form method="POST" action="{{ route('admin.report.product.destroy',$reply->id,'edit') }}">
                @csrf
                {{method_field('delete')}}
...
<section class="report-box" id="report-box-reply">
    <article class="report-list">
        ...
        <div class="report-footer">
            ...
            @enderror
            <form  style="width:100%;" method="POST" action="{{ route('admin.report.product.store') }}">
                @csrf
