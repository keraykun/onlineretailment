...
<section class="report-box">
    ...
    <article class="report-list">
        <div>
            <a href="{{ route('admin.report.user.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
            <a href="#report-box-reply"><i class="fa fa-envelope" aria-hidden="true"></i></a>
            <form method="POST" action="{{ route('admin.report.user.destroy',$report->id) }}">
                @csrf
                {{ method_field('delete') }}
                <button type="submit"><i class="fa fa-trash"></i></button>
             </form>
             <form method="POST" action="{{ route('admin.report.user.update',$report->id) }}">
                @csrf
                {{ method_field('PATCH') }}
    ...
    <article class="report-list" style="height: 0%;">
        <div class="report-header-reply">
          ...
          <div style="display: flex;">
            @can('update',$reply)
                 {{-- <a href=""><i style="margin: 0px !important; font-size:1.1rem; cursor: pointer;" class="fa fa-pencil"></i></a> --}}
            @endcan
            @can('delete',$reply)
            <form method="POST" action="{{ route('admin.report.user.destroy',$reply->id,'edit') }}">
                @csrf
                {{method_field('delete')}}
...
<section class="report-box" id="report-box-reply">
    <article class="report-list">
        ...
        <div class="report-footer">
            ...
            @enderror
            <form  style="width:100%;" method="POST" action="{{ route('admin.report.user.store') }}">
                @csrf
