    ...
    <article class="report-list">
        <div style="display: flex;">
            ...
            <a href="#report-box-reply"><i class="fa fa-envelope" aria-hidden="true"></i></a>
             {{-- <form method="POST" action="{{ route('client.report.violation.solve',$report->id) }}">
                @csrf
                {{ method_field('PATCH') }}
    ...
    <article class="report-list" style="height: 0%;">
        <div class="report-header-reply">
          ...
          <div style="display: flex;">
            ...
            @can('delete',$reply)
            <form method="POST" action="{{ route('client.report.violation.destroy',$reply->id) }}">
                @csrf
                {{method_field('delete')}}
...
<section class="report-box" id="report-box-reply">
    <article class="report-list">
        <div class="report-head">
            <h1>{{ $report->title }}</h1>
            <p><i>From</i> : {{ $report->touser->name }}</p>
            <small><i>Email</i> : {{ $report->touser->email }}</small>
            <span><i>Date</i> : {{ date('M d D h:i:s a',strtotime($report->created_at ))}}</span>
            <span><i>Role </i> :{{ ucfirst( $report->touser->role->name ) }}</span>
        </div>
        <div class="report-footer">
            @error('textarea')
               <span style="padding:4px; background:red; color:white;"> {{ $message }}</span>
            @enderror
             <form  style="width:100%;" method="POST" action="{{ route('client.report.violation.store') }}">
                @csrf
