...
<section class="report-box">
    ...
    <article class="report-list">
        <div style="display: flex;">
            <a href="{{ route('client.report.refund.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
            @if ($report->notification===1)
            <a href="#report-box-reply"><i class="fa fa-envelope" aria-hidden="true"></i></a>
             <form method="POST" action="{{ route('client.report.refund.solve',$report->id) }}">
                @csrf
                {{ method_field('PATCH') }}
        ...
        <div class="report-head" >
            <h1>{{ $report->title }}</h1>
            <p><i>From</i> : {{ $report->user->name }}</p>
            <small><i>Email</i> : {{ $report->user->email }}</small>
            <span><i>Date</i> : {{ date('M d D h:i:s a',strtotime($report->created_at ))}}</span>
            <span><i>Role </i> :{{ ucfirst( $report->user->role->name ) }}</span>
        </div>
        ...
        <div class="report-body" style=" flex-direction:column; align-items:flex-start; gap:30px;">
              ...
              <div class="report-list">
                 <img src="{{ asset('storage/order/'.$report->product->image->name) }}" alt="">
                 <div class="report-item">
                    <p><span><b>Item name</b> : </span>{{ $report->product->name }}</p>
                    <p><span><b>Item cateory</b> : </span>{{ $report->product->category->name }}</p>
                    <p><span><b>Item price</b> : </span>{{ number_format($report->product->price,2) }}</p>
                    @if ($report->notification==1)
                    <form method="POST" action="{{ route('client.report.refund.order',$report->id) }}">
                        @csrf
                        {{ method_field('PATCH') }}
                    <p><span><button class="btn btn-success" type="submit">Approve Refund </button> </p>
                    </form>
                    @endif
                </div>
              </div>
    ...
    <article class="report-list" style="height: 0%;">
        <div class="report-header-reply">
          ...
          <div style="display: flex;">
            ...
            @can('delete',$reply)
            <form method="POST" action="{{ route('client.report.refund.destroy',$reply->id) }}">
                @csrf
                {{method_field('delete')}}
...
<section class="report-box" id="report-box-reply">
    <article class="report-list">
        <div class="report-head">
            <h1>{{ $report->title }}</h1>
            <p><i>From</i> : {{ $report->owner->name }}</p>
            <small><i>Email</i> : {{ $report->owner->email }}</small>
            <span><i>Date</i> : {{ date('M d D h:i:s a',strtotime($report->created_at ))}}</span>
            <span><i>Role </i> :{{ ucfirst( $report->owner->role->name ) }}</span>
        </div>
        <div class="report-footer">
            @error('textarea')
               <span style="padding:4px; background:red; color:white;"> {{ $message }}</span>
            @enderror
             <form  style="width:100%;" method="POST" action="{{ route('client.report.refund.store') }}">
                <form method="POST" action="#">
                @csrf
