...
<section class="report-box">
    ...
    <article class="report-list">
        ...
        <div class="report-head">
            @switch($report->productsold->status)
            @case('refund')
                <span style="color:orange !important;">{{ ucfirst($report->productsold->status) }}</span>
                @break
            @case('delivered')
                <span style="color:green !important;">{{ ucfirst($report->productsold->status) }}</span>
                @break
            @default
        @endswitch
        </div>
        <div class="report-body">
            <a href="{{ route('client.report.refund.show',[$report->product_id,$report->from_user_id]) }}">View Report <i style="font-size: 1.1rem; margin-left:10px;" class="fa fa-eye"></i></a>
            @csrf
