...
<section class="report-box">
    ...
    <article class="report-list">
        <div style="display: flex;">
            <a href="{{ route('user.report.refund.index') }}"><i class="fa fa-arrow-left" aria-hidden="true"></i></a>
            @if(!is_null($report))
            @if ($report->notification==1)
            <a href="#report-box-reply"><i class="fa fa-envelope" aria-hidden="true"></i></a>
            <form method="POST" action="{{ route('user.report.refund.trash',$report->id) }}">
                @csrf
                {{ method_field('delete') }}
                <button type="submit"><i class="fa fa-trash"></i></button>
             </form>
             {{-- <form method="POST" action="{{ route('user.report.refund.solve',$report->id) }}">
                @csrf
                {{ method_field('PATCH') }}
    ...
    <article class="report-list" style="height: 0%;">
        <div class="report-header-reply">
          ...
          <div style="display: flex;">
            ...
            @can('delete',$reply)
            <form method="POST" action="{{ route('user.report.refund.destroy',$reply->id) }}">
                @csrf
                {{method_field('delete')}}
...
@if(!is_null($report->notification))
...
<section class="report-box" id="report-box-reply">
    <article class="report-list">
        ...
        <div class="report-footer">
            ...
            @enderror
            <form  style="width:100%;" method="POST" action="{{ route('user.report.refund.store') }}">
                <form method="POST" action="#">
                @csrf
