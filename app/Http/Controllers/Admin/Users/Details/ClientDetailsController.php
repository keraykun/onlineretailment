...
class ClientDetailsController extends Controller
{
    public function products(User $data){
        $products = Products::where('user_id',$data->id)
        ->with(['image','category'])->paginate(10);
       return view('admin.users.client.index',['products'=>$products,'data'=>$data]);
   }

   public function history(User $data){
      $products = productSold::where('user_id',$data->id)
      ->with(['product.category','product.image'])
      ->orderBy('_id','desc')
      ->paginate(8);
      return
      view('admin.users.client.history',['products'=>$products,'data'=>$data]);
   }
   ...
   public function show(User $data){
     ...
     return view('admin.users.client.violation_create',['data'=>$data]);
    }

   public function violation(User $data){
     $reports = violationReports::where('to_user_id',$data->_id)
    ->with(['fromuser.image'])->get();
    return view('admin.users.client.violation',['reports'=>$reports,'data'=>$data]);
   }
   ...
   public function message(violationReports $data){
      ...
      return view('admin.users.client.violation_message',['report'=>$report]);
   }
